...
#define SUPERcomplexMYownCONFIG_CONF    "conf-weWrotethis.conf"   // a lot of tables, tables are all maxed out -----implemented--------------
#define onlyTABLE    "fivecols"	// the tenth colomn inside the config we wrote --------------implemented-----------------------------
#define KEY5		"lala"	// A key used in the test cases.--------------implemented-----------------------------
#define KEY6		"lulu"	// A key used in the test cases.--------------implemented-----------------------------
#define KEY7		"lele"	// A key used in the test cases.--------------implemented-----------------------------
#define KEY8		"lili"	// A key used in the test cases.--------------implemented-----------------------------
#define KEY9		"lyly"	// A key used in the test cases.--------------implemented-----------------------------
#define KEY10		"lolo"	// A key used in the test cases.--------------implemented-----------------------------
#define KEY11		"outofconstants"	// A key used in the test cases.--------------implemented-----------------------------
#define KEY12		"lxlx"	// A key used in the test cases.--------------implemented-----------------------------
#define KEY13		"lzlz"	// A key used in the test cases.--------------implemented-----------------------------
...
/**
 * @brief perform a couple set operations
 */
START_TEST (test_set_add)
{
	// just add a correct entry
	struct storage_record record;
	int status;
	strncpy(record.value, "col1 5, col2 bob,col3 34,col4 helloo", sizeof record.value);
	status = storage_set(onlyTABLE, KEY9, &record, test_conn);
	fail_unless(status == 0, "Should Pass SuccessFully 1");

	// should succeed even with the multiple strings in col2
	strncpy(record.value, "col1 -16, col2 sam hi bye try, col3 -20,col4 byeee", sizeof record.value);
	status = storage_set(onlyTABLE, KEY11, &record, test_conn);
	fail_unless(status == 0, "Should Pass SuccessFully 2");

	// should fail due to the multiple integers in the first column
	strncpy(record.value, "col1 16 12,col2 sam  ,col3 -20,col4 byeee", sizeof record.value);
	status = storage_set(onlyTABLE, KEY10, &record, test_conn);
	fail_unless(status == -1, "Should Fail, invalid parameter");
	fail_unless(errno == ERR_INVALID_PARAM, "storage_set with multiple ints in one column should fail.");
}
END_TEST

/**
 * @brief perform a couple get operations
 */
START_TEST (test_get)
{
	// just add a correct entry
	...
	// check to see if the value for the key is correct
	status = storage_get(onlyTABLE, KEY9, &record, test_conn);
	if (strcmp(record.value, "col1 5, col2 bob,col3 34,col4 helloo") == 0){
		checker = 0; // success
	}
	else {
		checker = 1; //failure, the values do not match
	}	
	fail_unless(checker == 0, "Key set and get do not match");

	// check to see a get operation with an invalid key
	status = storage_get(onlyTABLE, KEY3, &record, test_conn);
...
/**
 * @brief set and modify
 */
START_TEST (test_setModify)
{
	// just add a record entry
	struct storage_record record1;
	strncpy(record1.value, "col1 5, col2 bob,col3 34,col4 helloo", sizeof record1.value);
	storage_set(onlyTABLE, KEY9, &record1, test_conn);
	storage_get(onlyTABLE, KEY9, &record1, test_conn);	

	// modify that entry
	struct storage_record record2;
	strncpy(record2.value, "col1 15, col2 bobSam,col3 343,col4 helloo", sizeof record2.value);
	storage_set(onlyTABLE, KEY9, &record2, test_conn);
	storage_get(onlyTABLE, KEY9, &record2, test_conn);

	int checker;
	if (strcmp(record1.value, record2.value)==0){
		checker= 1; //failure, they match after a modification
	}
	else checker = 0; // success, the modified value does not match the original

	fail_unless(checker == 0, "did not modify correctly");
}
END_TEST


/**
 * @brief set and delete
 */
START_TEST (test_setDelete)
{
	// just add a record entry
	struct storage_record record;
	strncpy(record.value, "col1 5, col2 bob,col3 34,col4 helloo", sizeof record.value);
	storage_set(onlyTABLE, KEY9, &record, test_conn);
	
	// the record is set to NULL so we cant delete the record at that key
	storage_set(onlyTABLE, KEY9, NULL, test_conn);

	// check to see if the key is still in the table
	int status = storage_get(onlyTABLE, KEY9, &record, test_conn);
	fail_unless(status == -1, "storage_get with missing key should fail.");
	fail_unless(errno == ERR_KEY_NOT_FOUND, "storage_get with missing key not setting errno properly.");
}
END_TEST
...
/**
 * @brief storage_query tests
 */
START_TEST (test_query_check_return)
{
	// just add a record entry
	struct storage_record record1;
	strncpy(record1.value, "col1 1, col2 bob,col3 10,col4 helloo", sizeof record1.value);
	storage_set(onlyTABLE, KEY5, &record1, test_conn);

	strncpy(record1.value, "col1 2, col2 bob,col3 11,col4 hellooo", sizeof record1.value);
	storage_set(onlyTABLE, KEY6, &record1, test_conn);

	strncpy(record1.value, "col1 3, col2 bob2,col3 34,col4 helloo", sizeof record1.value);
	storage_set(onlyTABLE, KEY7, &record1, test_conn);

	strncpy(record1.value, "col1 4, col2 bob3,col3 34,col4 helloo", sizeof record1.value);
	storage_set(onlyTABLE, KEY8, &record1, test_conn);

	strncpy(record1.value, "col1 5, col2 bob4,col3 34,col4 helloo", sizeof record1.value);
	storage_set(onlyTABLE, KEY9, &record1, test_conn);

	strncpy(record1.value, "col1 6, col2 bob5,col3 34,col4 helloo", sizeof record1.value);
	storage_set(onlyTABLE, KEY10, &record1, test_conn);

	strncpy(record1.value, "col1 7, col2 bob6,col3 34,col4 helloo", sizeof record1.value);
	storage_set(onlyTABLE, KEY11, &record1, test_conn);

	strncpy(record1.value, "col1 8, col2 bob7,col3 34,col4 helloo", sizeof record1.value);
	storage_set(onlyTABLE, KEY12, &record1, test_conn);

	// Do a query.  
	int foundkeys = storage_query(onlyTABLE, "col1 > 0, col2 = bob, col4 = helloo, col3 = 10", test_keys, MAX_RECORDS_PER_TABLE, test_conn);
	fail_unless(foundkeys == 1, "Query should find exactly 1 key");
}
END_TEST
...
void test_myConf_startServer()
{
	test_conn = init_start_connect(SUPERcomplexMYownCONFIG_CONF, "simpleempty.serverout", NULL);
	fail_unless(test_conn != NULL, "Couldn't start or connect to server.");
}
...
//--------------------------------UNIT TESTS---CD-051------------------------------
//TEST 1) storage_query
//TEST 2) storage_get
//TEST 3,4,5) storage_set (3)adding a record to an existing table (4) modifying a rec (5) deleting a rec

// TEST1  --- storage_query
	tc = tcase_create("storage_query tests");
	tcase_set_timeout(tc, TESTTIMEOUT);
	tcase_add_checked_fixture(tc, test_myConf_startServer, test_teardown);
	tcase_add_test(tc, test_query_check_return);
	suite_add_tcase(s, tc);

// ---------------------------------

	// TEST2  --- storage_get
	tc = tcase_create("Storage_get cases");
	tcase_set_timeout(tc, TESTTIMEOUT);
	tcase_add_checked_fixture(tc, test_myConf_startServer, test_teardown);
...
// ---------------------------------
	...
	// TEST3  --- storage_set ---------- adding a record
	tc = tcase_create("Storage_set ADDING A REC");
	tcase_set_timeout(tc, TESTTIMEOUT);
	tcase_add_checked_fixture(tc, test_myConf_startServer, test_teardown);
	tcase_add_test(tc, test_set_add);
	suite_add_tcase(s, tc);

// ---------------------------------

	// TEST4  --- storage_set ---------- modifying a record
	tc = tcase_create("Storage_set MODIFY AND STRCMP");
	tcase_set_timeout(tc, TESTTIMEOUT);
	tcase_add_checked_fixture(tc, test_myConf_startServer, test_teardown);
	tcase_add_test(tc, test_setModify);
	suite_add_tcase(s, tc);

// ---------------------------------

	// TEST5  --- storage_set---------- deleting a record
	tc = tcase_create("Storage_set deleting a record");
	tcase_set_timeout(tc, TESTTIMEOUT);
	tcase_add_checked_fixture(tc, test_myConf_startServer, test_teardown);
	tcase_add_test(tc, test_setDelete);
	suite_add_tcase(s, tc);
