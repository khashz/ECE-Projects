...
/**
 * @brief A struct to store each table's configuration type
 */

struct table_info {
	// will store the table name for line
	char table_names[MAX_TABLE_LEN];
	
	// names of each column
	char name_for_column[MAX_COLUMNS_PER_TABLE][MAX_COLNAME_LEN];
	
	// types for the corresponding names
	char type_for_name[MAX_COLUMNS_PER_TABLE][10];
};

/**
 * @brief A struct to store config parameters. Includes table names and the struct table_info
 */
 ...
 *
 * @param config_file The name of the configuration file./homes/l/loboweyl/Documents/storage/src/server.c:117: undefined reference to `set_function'
